from __future__ import annotations

from typing import Any

from ADSMOD.server.common.constants import (
    COLUMN_AIC,
    COLUMN_AICC,
    COLUMN_OPTIMIZATION_METHOD,
    COLUMN_SCORE,
)

###############################################################################
COLUMN_BETA_ERROR = "beta error"
COLUMN_EXPONENT_ERROR = "exponent error"
COLUMN_K_ERROR = "k error"
COLUMN_QSAT_ERROR = "qsat error"

MODEL_SCHEMAS: dict[str, dict[str, Any]] = {
    "LANGMUIR": {
        "prefix": "Langmuir",
        "table": "adsorption_langmuir",
        "fields": {
            "optimization_method": COLUMN_OPTIMIZATION_METHOD,
            "score": COLUMN_SCORE,
            "k": "k",
            "k_error": COLUMN_K_ERROR,
            "qsat": "qsat",
            "qsat_error": COLUMN_QSAT_ERROR,
            "aic": COLUMN_AIC,
            "aicc": COLUMN_AICC,
        },
    },
    "SIPS": {
        "prefix": "Sips",
        "table": "adsorption_sips",
        "fields": {
            "optimization_method": COLUMN_OPTIMIZATION_METHOD,
            "score": COLUMN_SCORE,
            "k": "k",
            "k_error": COLUMN_K_ERROR,
            "qsat": "qsat",
            "qsat_error": COLUMN_QSAT_ERROR,
            "exponent": "exponent",
            "exponent_error": COLUMN_EXPONENT_ERROR,
            "aic": COLUMN_AIC,
            "aicc": COLUMN_AICC,
        },
    },
    "FREUNDLICH": {
        "prefix": "Freundlich",
        "table": "adsorption_freundlich",
        "fields": {
            "optimization_method": COLUMN_OPTIMIZATION_METHOD,
            "score": COLUMN_SCORE,
            "k": "k",
            "k_error": COLUMN_K_ERROR,
            "exponent": "exponent",
            "exponent_error": COLUMN_EXPONENT_ERROR,
            "aic": COLUMN_AIC,
            "aicc": COLUMN_AICC,
        },
    },
    "TEMKIN": {
        "prefix": "Temkin",
        "table": "adsorption_temkin",
        "fields": {
            "optimization_method": COLUMN_OPTIMIZATION_METHOD,
            "score": COLUMN_SCORE,
            "k": "k",
            "k_error": COLUMN_K_ERROR,
            "beta": "beta",
            "beta_error": COLUMN_BETA_ERROR,
            "aic": COLUMN_AIC,
            "aicc": COLUMN_AICC,
        },
    },
    "TOTH": {
        "prefix": "Toth",
        "table": "adsorption_toth",
        "fields": {
            "optimization_method": COLUMN_OPTIMIZATION_METHOD,
            "score": COLUMN_SCORE,
            "k": "k",
            "k_error": COLUMN_K_ERROR,
            "qsat": "qsat",
            "qsat_error": COLUMN_QSAT_ERROR,
            "exponent": "exponent",
            "exponent_error": COLUMN_EXPONENT_ERROR,
            "aic": COLUMN_AIC,
            "aicc": COLUMN_AICC,
        },
    },
    "DUBININ_RADUSHKEVICH": {
        "prefix": "Dubinin-Radushkevich",
        "table": "adsorption_dubinin_radushkevich",
        "fields": {
            "optimization_method": COLUMN_OPTIMIZATION_METHOD,
            "score": COLUMN_SCORE,
            "qsat": "qsat",
            "qsat_error": COLUMN_QSAT_ERROR,
            "beta": "beta",
            "beta_error": COLUMN_BETA_ERROR,
            "aic": COLUMN_AIC,
            "aicc": COLUMN_AICC,
        },
    },
    "DUAL_SITE_LANGMUIR": {
        "prefix": "Dual-Site Langmuir",
        "table": "adsorption_dual_site_langmuir",
        "fields": {
            "optimization_method": COLUMN_OPTIMIZATION_METHOD,
            "score": COLUMN_SCORE,
            "k1": "k1",
            "k1_error": "k1 error",
            "qsat1": "qsat1",
            "qsat1_error": "qsat1 error",
            "k2": "k2",
            "k2_error": "k2 error",
            "qsat2": "qsat2",
            "qsat2_error": "qsat2 error",
            "aic": COLUMN_AIC,
            "aicc": COLUMN_AICC,
        },
    },
    "REDLICH_PETERSON": {
        "prefix": "Redlich-Peterson",
        "table": "adsorption_redlich_peterson",
        "fields": {
            "optimization_method": COLUMN_OPTIMIZATION_METHOD,
            "score": COLUMN_SCORE,
            "k": "k",
            "k_error": COLUMN_K_ERROR,
            "a": "a",
            "a_error": "a error",
            "beta": "beta",
            "beta_error": COLUMN_BETA_ERROR,
            "aic": COLUMN_AIC,
            "aicc": COLUMN_AICC,
        },
    },
    "JOVANOVIC": {
        "prefix": "Jovanovic",
        "table": "adsorption_jovanovic",
        "fields": {
            "optimization_method": COLUMN_OPTIMIZATION_METHOD,
            "score": COLUMN_SCORE,
            "k": "k",
            "k_error": COLUMN_K_ERROR,
            "qsat": "qsat",
            "qsat_error": COLUMN_QSAT_ERROR,
            "aic": COLUMN_AIC,
            "aicc": COLUMN_AICC,
        },
    },
}

