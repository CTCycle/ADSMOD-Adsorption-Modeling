...
from ADSMOD.server.utils.services.loader import SCADSDataLoader
...
class AdsorptionPredictions:
    ...
    def process_inference_output(
        self, inputs: dict[str, np.ndarray], predictions: np.ndarray
    ) -> list[Any]:
    ...
    def build_predictions_dataset(
        self, data: pd.DataFrame, predictions: list[np.ndarray | Any]
    ) -> pd.DataFrame:
