...
def resolve_conflict_columns(table: Table) -> list[str]:
    ...
    if not unique_constraints:
        ...
        raise ValueError(
            f"No unique or primary key constraint found for {table.name}"
        )
    ...
    def candidate_rank(columns: list[str]) -> tuple[int, int, int, tuple[int, ...], tuple[str, ...]]:
        ...
        ordered_positions = tuple(column_positions.get(column, 10000) for column in columns)
        return (
