...
fn render_startup_screen(
    ...
    let spinner_markup = if show_spinner {
        format!(
            "<div style='display:flex;align-items:center;gap:14px;margin:0 0 20px 0;'><div style='width:22px;height:22px;border:3px solid rgba(226,232,240,0.20);border-top-color:{accent};border-right-color:{accent};border-radius:50%;animation:adsmod-spin 0.85s linear infinite;'></div><div style='font-size:12px;letter-spacing:0.16em;text-transform:uppercase;color:#94a3b8;'>Launching services</div></div>"
        )
    } else {
        String::new()
    };
    ...
    let script = format!(
        "document.body.style.margin='0';document.body.style.background='radial-gradient(circle at top, #16213a 0%, #0f172a 58%, #020617 100%)';document.body.style.color='#e2e8f0';document.body.style.fontFamily='Segoe UI, sans-serif';document.body.innerHTML=\"<style>@keyframes adsmod-spin{{from{{transform:rotate(0deg)}}to{{transform:rotate(360deg)}}}}</style><div style='min-height:100vh;display:flex;align-items:center;justify-content:center;padding:32px;box-sizing:border-box;'><div style='max-width:720px;width:100%;background:rgba(15,23,42,0.82);backdrop-filter:blur(12px);border:1px solid rgba(148,163,184,0.20);border-radius:22px;padding:32px 34px;box-shadow:0 28px 70px rgba(0,0,0,0.42);'>{spinner_markup}<h2 style='margin:0 0 12px 0;font-size:30px;font-weight:600;'>\"+ '{escaped_title}' +\"</h2><pre style='margin:0;white-space:pre-wrap;line-height:1.6;font-size:15px;color:#cbd5e1;'>\"+ '{escaped_message}' +\"</pre></div></div>\";"
    );
...
fn directory_is_writable(path: &Path) -> bool {
    ...
    let probe_path = path.join(".adsmod-write-probe");
...
fn spawn_backend(app_handle: &tauri::AppHandle, state: &BackendChildState) -> Result<(), String> {
    #[cfg(not(target_os = "windows"))]
    {
        ...
        return Err(String::from(
            "Local desktop mode is currently supported on Windows only.",
        ));
    }
    ...
    #[cfg(target_os = "windows")]
    {
        let workspace_root = find_workspace_root(app_handle).ok_or_else(|| {
            String::from("Cannot resolve packaged backend workspace (missing pyproject.toml/ADSMOD).")
        })?;
