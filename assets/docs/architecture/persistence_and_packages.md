...
- Typed repositories (`datasets`, `materials`, `fitting`, and `nist`) own explicit conflict targets and SQL projections. `repositories/nist.py` is the single SQLAlchemy owner for canonical NIST reads and writes; `repositories/queries` contains training-specific query helpers.
...
NIST, fitting, and training workflows. `core_service.services.data.nist_mapper`
converts external NIST frames before they cross into the shared NIST repository.
Public-source pages reuse this persistence boundary without adding
