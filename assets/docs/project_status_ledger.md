...
| --- | --- | --- | --- |
| Tier 0 — environment and startup | `ADS-T0-01`, `ADS-T0-02` | `PARTIAL` / `FAIL` | Run `35620065349` created all CI jobs but exposed hosted uv/Edge setup failures; launcher occupied-port refusal must also be repaired and rerun. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ADS-T0-01` | Current GitHub Actions quality gates | YES | PARTIAL | `PARTIAL` | `cec373a1` → follow-up pending | Workflow evaluation is fixed, but run `35620065349` failed backend Python provisioning and all seven frontend visual projects in hosted setup | [`tier-0-summary.md`](../QA/validation/2026-09-21/tier-0-summary.md); [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md) | Push the uv `0.11.31` and Playwright `/tmp` remediation, then confirm every job step completes | remote diagnostic + job logs + source diff |
...
| `quality.validation-gates` | `PARTIAL` | Backend/frontend tests, lint, build, generated contracts, visual checks, and repository-local cache layout. | [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md) records the workflow-definition repair and run `35620065349` job logs; frontend ordinary checks passed but hosted backend/browser setup failed. | The remote gate is executable but still red: uv `0.11.30` could not provision Python `3.14.7`, and Edge rejected the long canonical temp socket path. Protected pytest-cache directories can also produce Windows access-denied cleanup warnings. | Follow-up remote run plus current-checkout cache/basetemp confirmation. | 2026-09-21 (Tier 0 hosted setup diagnosis) | Remote workflow diagnostic + job logs + source diff | [`validation/roadmap.md`](validation/roadmap.md); [`coding/quality_gates.md`](coding/quality_gates.md); [`operations/commands.md`](operations/commands.md); [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md) | Verify backend installs and all seven visual projects after the uv `0.11.31` and `/tmp` changes. |
...
| `ISSUE-007` | `quality.validation-gates` | `MEDIUM` | The first post-definition-fix CI run scheduled all jobs but failed backend dependency provisioning and all seven frontend visual projects in hosted-runner setup. | Current remote CI remains red even though workflow evaluation and ordinary frontend checks are executable. | [Run 35620065349](https://github.com/CTCycle/ADSMOD-Adsorption-Modeling/actions/runs/35620065349) recorded `uv 0.11.30` unable to find Python `3.14.7` and Edge `Socket path too long` under the canonical TMPDIR. | CI uv pin lagged the repository-local runtime; Playwright inherited a repository path too long for Linux's browser singleton socket. | Follow-up CI-only remediation and rerun. | Open validation/configuration issue; no product defect inferred. | Verify base/ML installs, seven visual projects, and all remaining steps after the uv `0.11.31` and `/tmp` changes. | [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md); [`validation/roadmap.md`](validation/roadmap.md) |
...
| --- | --- | --- | --- |
| `ADS-T0-01` | Medium / partial | Confirm the uv `0.11.31` backend provisioning and Playwright `/tmp` remediation across all three remote jobs, then record the final run. | Critical |
