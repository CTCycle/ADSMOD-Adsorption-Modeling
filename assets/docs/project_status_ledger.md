...
The current Tier 0 validation baseline is `develop` at committed SHA
`2eb3bc13823bedae1be1791fc980a78613d00ef1`. The exact hosted workflow run is
`35725502460`; the official Windows launcher and profile evidence use the same
implementation SHA. Tracked launcher/frontend changes are committed. Pre-
existing untracked cache residue, protected paths, and unrelated data were
preserved and are not part of the evidence claim.
...
is now closed; Tier 1 and later tiers remain untested.

| Tier | Slice IDs | Status | Current gate |
| --- | --- | --- | --- |
| Tier 0 — environment and startup | `ADS-T0-01`, `ADS-T0-02` | `PASS` / `PASS` | Exact SHA `2eb3bc13823bedae1be1791fc980a78613d00ef1` passed hosted CI run `35725502460`; the official launcher lifecycle, conflict/race matrix, invalidation matrix, ML-profile repair, and early-failure cleanup are current. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ADS-T0-01` | Current GitHub Actions quality gates | YES | YES | `PASS` | `2eb3bc13823bedae1be1791fc980a78613d00ef1` | None in the required gate; hosted annotations are platform maintenance warnings only | [`tier-0-summary.md`](../QA/validation/2026-09-22/tier-0-summary.md); [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-22/ADS-T0-01/ci-workflow.md); [hosted run 35725502460](https://github.com/CTCycle/ADSMOD-Adsorption-Modeling/actions/runs/35725502460) | — | hosted CI + local exact gates |
...
| `quality.validation-gates` | `VALIDATED` | Backend/frontend tests, lint, build, generated contracts, visual checks, profile-correct runner behavior, and repository-local cache layout. | [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-22/ADS-T0-01/ci-workflow.md) records exact local gates plus hosted run `35725502460` on SHA `2eb3bc1`; the workflow passed `test:preview` and all 7 Edge projects. | Protected pre-existing Windows cache directories can produce local ACL warnings; the hosted gate and repository-local override both pass. | — | 2026-09-22 | Hosted CI + local automated gates | [`validation/roadmap.md`](validation/roadmap.md); [`coding/quality_gates.md`](coding/quality_gates.md); [`operations/commands.md`](operations/commands.md); [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-22/ADS-T0-01/ci-workflow.md); [`ADS-T0-02/launcher-lifecycle.md`](../QA/validation/2026-09-22/ADS-T0-02/launcher-lifecycle.md) | Revalidate the three-job workflow and affected profile suites after future changes. |
...
| `ISSUE-007` | `quality.validation-gates` | `MEDIUM` | The hosted CI workflow briefly failed before backend validation because `setup-uv` selected a Python version without installing it on the runner. | No current impact after the explicit Python setup remediation; the current CI gate is green. | [Current run 35725502460](https://github.com/CTCycle/ADSMOD-Adsorption-Modeling/actions/runs/35725502460) passed all three jobs; earlier runs preserve the `No interpreter found for Python 3.14.7` evidence. | `setup-uv`'s `python-version` input selected `UV_PYTHON` but did not supply the hosted interpreter; `actions/setup-python@v5` now does. | — | Resolved; current SHA passed the complete workflow. | Repeat the three-job gate after future workflow/runtime/dependency changes; watch the Node.js 20 and `ubuntu-latest` migration annotations. | [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-22/ADS-T0-01/ci-workflow.md); [`validation/roadmap.md`](validation/roadmap.md) |
## Validation debt
...
| `quality.validation-gates` | High / current hosted pass | No remaining Tier 0 gate debt; revalidate after future workflow, dependency, or generated-contract changes. | — |
