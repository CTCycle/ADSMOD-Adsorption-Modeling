...
The baseline inspected for this ledger is `develop` at `717f6034`. The
application worktree was clean before the current runtime update. Evidence
that predates a later change to the same component is called out explicitly
and does not qualify as current `VALIDATED` evidence by itself.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `runtime.startup.windows` | `WORKING` | Official Windows launcher, one backend/frontend lifecycle, port ownership, browser opening, stop, and application-process cleanup. | [`adsmod-end-to-end-audit-2026-09-10.md`](../QA/adsmod-end-to-end-audit-2026-09-10.md) passed launcher lifecycle and occupied-port checks; [`adsmod-python-3147-upgrade-20260921.md`](../QA/adsmod-python-3147-upgrade-20260921.md) passed the current Python 3.14.7 start/readiness/stop smoke. | The current smoke did not re-run occupied-port protection or browser-opening assertions, so the full scope remains `WORKING`. | — | 2026-09-21 (Python 3.14.7 upgrade smoke) | Manual E2E (upgrade smoke) | [`runtime/startup.md`](runtime/startup.md); [`runtime/deployment.md`](runtime/deployment.md) | Re-run start/stop/start/stop, occupied-port protection, cleanup, readiness, and final port checks on the current checkout. |
...
| `quality.validation-gates` | `WORKING` | Backend/frontend tests, lint, build, generated contracts, visual checks, and repository-local cache layout. | The 2026-09-16 package-cutover validation recorded current backend/unit/E2E, Ruff, lockfile, wheel, and generated-contract checks; the 2026-09-10 audit recorded frontend and visual gates; commits on 2026-09-18 added centralized cache handling and a cache-layout test. | Protected pytest-cache directories under `app/server/core` and `app/tests` have produced access-denied cleanup warnings in prior runs. | Windows ACLs can prevent cleanup or a full runner from using a global temp/cache location. | 2026-09-16 (backend; cache changes are newer) | Unit + integration + E2E + visual (prior baseline) | [`coding/quality_gates.md`](coding/quality_gates.md); [`operations/commands.md`](operations/commands.md); [`../../app/tests/unit/test_cache_layout.py`](../../app/tests/unit/test_cache_layout.py); [`adsmod-end-to-end-audit-2026-09-10.md`](../QA/adsmod-end-to-end-audit-2026-09-10.md) | Run the repository-local cache/basetemp gates on the current checkout and confirm the new cache ownership rules. |
