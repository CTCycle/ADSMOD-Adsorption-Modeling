...
The current validation baseline for this ledger is `develop` at `7ac893767b74`
with uncommitted launcher/frontend working-tree changes recorded in the linked
ADS-T0-02 evidence. The previous CI workflow gate was evidenced at
`35622186997`, but the workflow now includes an additional preview-server
test and has not been rerun remotely. Unrelated cache paths were preserved.
Evidence that predates a later change to the same component is called out
explicitly and does not qualify as current `VALIDATED` evidence by itself.
...
| --- | --- | --- | --- |
| Tier 0 — environment and startup | `ADS-T0-01`, `ADS-T0-02` | `PARTIAL` / `PARTIAL` | Frontend/launcher checks pass locally, but the new workflow has no remote rerun and the new launcher race/failure/profile scenarios remain outside current evidence. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ADS-T0-01` | Current GitHub Actions quality gates | YES | PARTIAL | `PARTIAL` | `7ac893767b74` | The existing remote run passed before this workflow gained `test:preview`; local lint, frontend unit, preview test, and build passed, but the hosted workflow was not rerun | [`tier-0-summary.md`](../QA/validation/2026-09-21/tier-0-summary.md); [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md); [`ADS-T0-02/launcher-lifecycle.md`](../QA/validation/2026-09-21/ADS-T0-02/launcher-lifecycle.md) | Rerun the three-job workflow after the working-tree changes | local frontend gates + prior remote workflow |
| `ADS-T0-02` | Official Windows launcher lifecycle and static preview | YES | PARTIAL | `PARTIAL` | `7ac893767b74` | Warm reuse, static `/datasets`, local health/capability proxying, aggregate decline/approval, one-PID and two-PID conflict handling, stop, and final free ports passed | [`ADS-T0-02/launcher-lifecycle.md`](../QA/validation/2026-09-21/ADS-T0-02/launcher-lifecycle.md) | Exercise unresolved/failed termination races, package-lock/build-state invalidation, ML profile preservation, and rerun after commit | live launcher + browser + HTTP + process/port checks |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `runtime.startup.windows` | `PARTIAL` | Official Windows launcher, static preview, backend/frontend lifecycle, aggregate port ownership, browser opening, stop, relaunch, and application-process cleanup. | [`ADS-T0-02/launcher-lifecycle.md`](../QA/validation/2026-09-21/ADS-T0-02/launcher-lifecycle.md) records warm reuse, browser `/datasets`, local health/capability proxying, aggregate decline/approval, one-PID/two-PID conflict handling, and final free ports. | Owner-unresolved, failed-termination, replacement-race, and new-listener final-recheck cases remain unvalidated; Chrome blocks direct JSON-only navigation. | Current evidence does not cover every new failure branch. | 2026-09-21 | Manual E2E + browser + HTTP + process/port checks | [`validation/roadmap.md`](validation/roadmap.md); [`runtime/startup.md`](runtime/startup.md); [`ADS-T0-02/launcher-lifecycle.md`](../QA/validation/2026-09-21/ADS-T0-02/launcher-lifecycle.md) | Complete the listed failure/race cases and rerun after commit. |
...
| `quality.validation-gates` | `PARTIAL` | Backend/frontend tests, lint, build, generated contracts, visual checks, and repository-local cache layout. | The prior remote run `35622186997` passed before the workflow change; current local frontend lint, unit, preview, build, launcher contract, and parser checks passed. | `app\tests\run_tests.bat` was not fully green in the local Standard/Base campaign because four ML-dependent unit modules lacked `sklearn`/`keras` and one Base-profile E2E navigation check lacked the ML-only Training link. The new hosted workflow was not rerun. | A current remote three-job run and an ML-profile local suite are still needed for a full current gate claim. | 2026-09-21 | Local targeted gates + prior remote workflow | [`validation/roadmap.md`](validation/roadmap.md); [`coding/quality_gates.md`](coding/quality_gates.md); [`operations/commands.md`](operations/commands.md); [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md); [`ADS-T0-02/launcher-lifecycle.md`](../QA/validation/2026-09-21/ADS-T0-02/launcher-lifecycle.md) | Rerun hosted CI and the appropriate Base/ML profile suites after the working-tree changes. |
...
| `ISSUE-007` | `quality.validation-gates` | `MEDIUM` | The hosted CI workflow briefly failed before backend validation because `setup-uv` selected a Python version without installing it on the runner. | No current impact after the explicit Python setup remediation; the final CI gate is green. | [Final run 35622186997](https://github.com/CTCycle/ADSMOD-Adsorption-Modeling/actions/runs/35622186997) passed all three jobs; [rerun 35621291855](https://github.com/CTCycle/ADSMOD-Adsorption-Modeling/actions/runs/35621291855) preserves the preceding `No interpreter found for Python 3.14.7` evidence. | `setup-uv`'s `python-version` input selected `UV_PYTHON` but did not supply the hosted interpreter; `actions/setup-python@v5` now does. | — | Resolved by `b52096bcb153a7f5724493a49374d30d352c8a6`; final remote run passed. | Repeat the three-job gate after future workflow/runtime/dependency changes; watch the Node.js 20 and `ubuntu-latest` migration annotations. | [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md); [`validation/roadmap.md`](validation/roadmap.md) |
## Validation debt
...
| --- | --- | --- | --- |
| `ADS-T0-01` | Medium / prior remote pass | New `test:preview` workflow step and current-checkout workflow inputs need a remote rerun. | Medium |
| `ADS-T0-02` | Medium / targeted current pass | Owner-unresolved/failed-termination/race cases, dependency/build invalidation, ML profile preservation, and post-commit lifecycle need evidence. | Medium |
| `runtime.startup.windows` | Medium / targeted current pass | Complete the launcher failure/race matrix and confirm current hosted gates after the startup changes. | Medium |
...
| `quality.validation-gates` | Medium / prior remote pass plus current targeted local gates | Remote workflow rerun and profile-correct full backend validation after the launcher/frontend changes. | Medium |
...
| The launcher could miss an active TCP listener during interactive occupied-port preflight. | Added an `IPGlobalProperties.GetActiveTcpListeners()` fallback that fails closed when the owner PID cannot be resolved; official-menu checks for controlled 6045 and 5173 listeners, clean relaunch, and final listener cleanup passed. | [`ADS-T0-02/launcher-lifecycle.md`](../QA/validation/2026-09-21/ADS-T0-02/launcher-lifecycle.md); [`../../start_on_windows.ps1`](../../start_on_windows.ps1) |
