...
| --- | --- | --- | --- |
| Tier 0 — environment and startup | `ADS-T0-01`, `ADS-T0-02` | `PARTIAL` / `FAIL` | Run `35621291855` confirms frontend/visual success but backend interpreter provisioning remains; launcher occupied-port refusal must also be repaired and rerun. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `ADS-T0-01` | Current GitHub Actions quality gates | YES | PARTIAL | `PARTIAL` | `436b77b7` → follow-up pending | Workflow evaluation, frontend checks, and all seven visual projects pass in rerun `35621291855`; both backend jobs still fail hosted Python provisioning | [`tier-0-summary.md`](../QA/validation/2026-09-21/tier-0-summary.md); [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md) | Add hosted `actions/setup-python@v5` for `3.14.7`, rerun, and confirm every job step completes | remote diagnostic + job logs + source diff |
...
| `quality.validation-gates` | `PARTIAL` | Backend/frontend tests, lint, build, generated contracts, visual checks, and repository-local cache layout. | [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md) records the workflow-definition repair and rerun `35621291855`; frontend install/checks/build and all seven visual projects passed, but hosted backend setup failed. | The remote gate is executable but still red because `setup-uv` selected `UV_PYTHON=3.14.7` without placing that interpreter on PATH. Protected pytest-cache directories can also produce Windows access-denied cleanup warnings. | Follow-up remote run plus current-checkout cache/basetemp confirmation. | 2026-09-21 (Tier 0 hosted setup diagnosis) | Remote workflow diagnostic + job logs + source diff | [`validation/roadmap.md`](validation/roadmap.md); [`coding/quality_gates.md`](coding/quality_gates.md); [`operations/commands.md`](operations/commands.md); [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md) | Verify both backend installs and all downstream steps after adding `actions/setup-python@v5` for `3.14.7`. |
...
| `ISSUE-007` | `quality.validation-gates` | `MEDIUM` | The hosted CI workflow now executes, but both backend jobs still fail before validation because the requested Python interpreter is not installed on the runner. | Current remote CI remains red even though frontend install/checks/build and all seven visual projects pass. | [Rerun 35621291855](https://github.com/CTCycle/ADSMOD-Adsorption-Modeling/actions/runs/35621291855) recorded `UV_PYTHON: 3.14.7` followed by `No interpreter found for Python 3.14.7`; the same run passed the complete frontend job. | `setup-uv` selected the requested version but did not install it; the workflow needs an explicit hosted Python setup step. | Follow-up CI-only remediation and rerun. | Open validation/configuration issue; no product defect inferred. | Verify base/ML installs and all remaining steps after adding `actions/setup-python@v5` for `3.14.7`. | [`ADS-T0-01/ci-workflow.md`](../QA/validation/2026-09-21/ADS-T0-01/ci-workflow.md); [`validation/roadmap.md`](validation/roadmap.md) |
...
| --- | --- | --- | --- |
| `ADS-T0-01` | Medium / partial | Confirm hosted Python `3.14.7` provisioning across both backend jobs, then record the final run after the frontend visual path is already green. | Critical |
