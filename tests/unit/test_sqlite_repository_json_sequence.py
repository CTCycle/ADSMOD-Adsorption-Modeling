...
def test_sqlite_prepare_for_save_serializes_sequences_to_json_strings() -> None:
    ...
    prepared = repository.prepare_for_sqlite_save(raw, TrainingDataset)

    assert isinstance(prepared.loc[0, "pressure"], str)
    assert isinstance(prepared.loc[0, "adsorbed_amount"], str)
    assert isinstance(prepared.loc[0, "adsorbate_encoded_smile"], str)

    assert SQLiteRepository.parse_json_column_value(prepared.loc[0, "pressure"]) == [
        1.0,
        2.0,
        3.0,
    ]
    assert SQLiteRepository.parse_json_column_value(
        prepared.loc[0, "adsorbed_amount"]
    ) == [0.1, 0.2, 0.3]
    assert SQLiteRepository.parse_json_column_value(
