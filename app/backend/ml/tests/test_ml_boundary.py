...
def test_ml_runtime_contracts() -> None:
    config = load_config(CONFIG_PATH).model_copy(
        update={
            "runtime": load_config(CONFIG_PATH).runtime.model_copy(
    ...
    with TestClient(create_app(config)) as client:
        ...
        assert (
            client.get("/api/v1/system/capabilities").json()["features"]["training"]
            is True
        )
...
def test_snapshot_client_fetches_pages_and_verifies_hash() -> None:
    ...
    content_hash = hashlib.sha256(
        json.dumps(
            [{"id": 1}, {"id": 2}], separators=(",", ":"), sort_keys=True
        ).encode()
    ).hexdigest()
    ...
    result = CoreSnapshotClient(
        "http://core", "secret", httpx.MockTransport(handler)
...
def test_snapshot_client_rejects_hash_mismatch() -> None:
    transport = httpx.MockTransport(
        lambda request: httpx.Response(
            200, json={"content_hash": "bad", "total_rows": 1, "rows": [{"id": 1}]}
        )
    )
    try:
        CoreSnapshotClient("http://core", "secret", transport).fetch_snapshot(
            "snapshot"
        )
