...
set "VENV_PYTHON=%PROJECT_ROOT%\\app\\server\\.venv\\Scripts\\python.exe"
set "NODEJS_DIR=%PROJECT_ROOT%\\runtimes\\nodejs"
set "NPM_CMD=%NODEJS_DIR%\\npm.cmd"
set "FRONTEND_DIR=%PROJECT_ROOT%\\app\\client"
...
set "BACKEND_URL=http://%BACKEND_CLIENT_HOST%:%FASTAPI_PORT%"
set "BACKEND_DOCS_URL=%BACKEND_URL%/docs"
set "FRONTEND_URL=http://%FRONTEND_CLIENT_HOST%:%UI_PORT%"
...
if "%BACKEND_RUNNING%"=="0" (
    ...
    start "" /B /D "%PROJECT_ROOT%" "%PYTHON_CMD%" -m uvicorn app.server.app:app --host !FASTAPI_HOST! --port !FASTAPI_PORT!
...
cd /d "%PROJECT_ROOT%"
"%PYTHON_CMD%" -m pytest "%PROJECT_ROOT%\\app\\tests" -v --tb=short %*
