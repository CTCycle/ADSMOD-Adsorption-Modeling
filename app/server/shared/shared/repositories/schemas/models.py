...
class FittingRun(Base):
    ...
    best_result_id: Mapped[int | None] = mapped_column(Integer)
    created_at: Mapped[datetime] = mapped_column(UTCDateTime, default=utc_now, nullable=False)
...
class TrainingDataset(Base):
    ...
    vocabularies: Mapped[dict[str, Any]] = mapped_column(
        JSONMapping, nullable=False, default=dict
    )
    normalization_stats: Mapped[dict[str, Any]] = mapped_column(
        JSONMapping, nullable=False, default=dict
    )
    created_at: Mapped[datetime] = mapped_column(UTCDateTime, default=utc_now, nullable=False)
...
class TrainingSample(Base):
    ...
    source_isotherm_id: Mapped[int | None] = mapped_column(
        ForeignKey("isotherms.id", ondelete="SET NULL")
    )
    ...
    training_dataset: Mapped[TrainingDataset] = relationship(
        back_populates="samples", lazy="raise"
    )
    ...
    __table_args__ = (
        ...
        Index("ix_training_samples_dataset_split", "training_dataset_id", "split"),
    )
